<template>
  <v-container class="section-expansion-panels" fluid>
    ...
    <v-row justify="center">
      <v-expansion-panels
        v-model="panel"
        class="c-expansion-panels"
        focusable
        multiple
        title
      >
        <slot name="panel">
          <v-expansion-panel v-for="(item, index) in compoList" :key="index">
            ...
            <v-expansion-panel-content class="pa-0">
              <v-container fluid>
                <component :id="item.name" :is="item.name" v-on="$listeners" />
              </v-container>
            </v-expansion-panel-content> </v-expansion-panel></slot></v-expansion-panels></v-row
  ></v-container>
</template>
<script>
export default {
